...
const TagListWizard1 = ({ onCancel, formValues, submitTagList, history, getRelatedVideosHandler, videoList }) => {
    ...
    const form_buttons = () => {
        return(
            <div>
                ...
                <button onClick={() => getRelatedVideosHandler(formValues['title'])} className="teal btn-flat right white-text">
                    Next
    ...
    const generateChoices = _.map(videoList.data, ({ title, videoId, videoURL, thumbnail, channelTitle, channelURL }) => {
        return (
            <div key={videoId} className='soft-outter'>
                <div className='soft-outter card col s12 m5 l5 offset-m1 offset-l1' style={{backgroundColor: "var(--softBackground)"}}>
